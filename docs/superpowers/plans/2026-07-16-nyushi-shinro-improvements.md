...
## 実施状況（2026-07-16 第2セッション）
以下を実装済み（テンプレ `work/app_v2ui.html`、4担当レビュー＋修正込み）:
- **進路タブ**: summary/noteリード、ratings 8分野横棒（診断職種🎯強調・「本アプリ独自の目安」明記）、警察官/消防・救命行（/要確認/除外）、注意点(emp.notes)、注力プログラム(重複除去)、キャリアセンターの一言、卒業生の活躍例、就職率リテラシー注記、資格・採用試験の強み（string/object構造化）、留学（2スキーマ吸収）
- **入試タブ**: 評定条件チップ（自分の評定と✅/⚠️照合つき）、日程横タイムライン（帯=出願期間・📝試験・🌸発表・⏰手続〆・◀8月以前・点線=終了日不明、月軸96px統一）、想定倍率ヒートバッジ（≤100ガード・色凡例つき）
- **学部ページ**: 偏差値出典リンク+検証日+50%注記、倍率前年比バッジ（連続2年を動的抽出・「倍率アップ/ダウン」表記）、同レベル併願候補（**実効偏差値=min+国公立8で比較**・ページ末尾配置・平易な注記）
- **概要タブ**: unique_point「ここが違う」行を復活（110/123大学）

## 実施状況（2026-07-17 第2バッチ: 全画面改善）
全画面探索ワークフロー（7エリア生成→データ実証批評＋未使用フィールド棚卸し）の採用案を4パックで実装済み:
- **①バグ修正**: 診断の地域/通学文言を「有利」→「絞り込み」に是正＋対象大学数を動的表示、偏差値クイックボタン×模試換算の二重適用防止、結果ゾーン件数の正直表示＋「残り◯件」遅延展開、Step3虚偽文言（実就職率）削除、favs死にID分離＋整理ボタン、学部ページの入試方式〇✕表示
- **②スキーマ吸収**: 生活タブ全面改修（4年間費用シミュ・東海圏旧スキーマOR吸収で最寄駅/周辺/OC/奨学金が123大学化・特待生/寮/男女比/出身地/雰囲気/食費内訳の行追加）、進路タブ就職先union（+270社実名）、ゼミ出典リンク
- **③結果カード**: 理由チップ具体化（📚法律・政治に一致 等）、国公私＋🚃滋賀から◯分/🏘️下宿向きタグ
- **④診断UX**: 評定スライダーに出願可能入試のライブカウンタ、興味チップに学部数バッジ＋希釈注意、RIASEC部分回答の警告、診断内←戻るの真の戻る化（進むボタン復活）
- 素人目線レビュー反映: 就職地域の説明補強、「78件（54大学）」表記と免責、入試方式見出しの矛盾解消、🏠/🏘️アイコン統一、費用シミュの内訳明示

### 第2バッチで採用済み・未実装（次回候補、批評ADOPT/FIX済み）
- **cross**: Android/ブラウザ戻る対応（history API、pushState try/catch）、ビルド時JSONミニファイ（1.95MB→約1.52MB、build_v2ui.js 1行）、@media print印刷対応＋🖨️ボタン、データ鮮度の全画面表示＋期限切れ警告、戻る時のスクロール位置復帰、フォーカス可視化（:focus-visible＋遷移時フォーカス管理）
- **favs**: 難易度順ソートチップ、📋テキストコピー（三者面談共有用）
- **list**: 興味分野チップ絞り込み（診断を経ない生徒の動線）
- **result**: 空結果の一発緩和ボタン（excluded計数の活用）
- 生活タブbairitsu_trend移植、タイムライン今日ライン等は前バッチ持ち越しと同様

### レビューからの持ち越し課題（未実装）
- .emp-ratings と既存 .rating-bar の二重デザイン統合（同一データの見た目が画面で異なる）
- region-badge系の白文字×地域色コントラスト（8地域中6がAA未達。既存からの全体課題）
- タイムラインに「▼今日」ライン＋診断で選んだ方式の🎯強調
- 併願候補行に🚀⚖️🛡️ゾーンチップ＋❤️お気に入り（button入れ子回避のためspan必須）
- 生活タブ: dormitory(34)/sports_tokutai(29)/campus_atmosphere(56)/gender_ratio(45)/nearby_amenities(83)/living_cost.note(123) の追加、bairitsu_trend(70大学)の移植
- ippan評定不変条件（全null）のビルド時アサーション
- 進路タブが縦長化 → dl後半のdetails折畳み検討
